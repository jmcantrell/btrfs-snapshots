load_profile() {
    ...
    if [[ ! -r $PROFILE_FILE ]]; then
        error "$TEXT_PROFILE_NOT_EXIST" PROFILE="$PROFILE_NAME"
        return 1
    fi
    ...
    for variable in SUBVOLUME SNAPSHOTS; do
        if [[ ! -v $variable || -z ${!variable} ]]; then
            error "$TEXT_PROFILE_NOT_SET" VARIABLE="$variable" PROFILE="$PROFILE_NAME"
            return 1
        fi

        if [[ ! ${!variable} =~ ^/.* ]]; then
            error "$TEXT_PROFILE_ABS_PATH" VARIABLE="$variable" PROFILE="$PROFILE_NAME"
            return 1
        fi
    done
}
